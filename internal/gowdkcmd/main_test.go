...
func TestBuildCommandUsesProjectHelperForImportableDynamicConfig(t *testing.T) {
	root := t.TempDir()
	writeCLITestModule(t, root, "example.com/dynamic-config")
	config := filepath.Join(root, "gowdk.config.go")
	writeCLIFile(t, config, `package app

import "github.com/cssbruno/gowdk"

func sourceIncludes() []string {
	return []string{"pages/*.page.gwdk"}
}
...
view {
  <main>dynamic config</main>
}
`)

	if err := runInDir(root, func() error {
		return run([]string{"build", "--config", config})
	}); err != nil {
		t.Fatal(err)
	}
...
func TestProjectHelperSourceUsesNativeConfigPointer(t *testing.T) {
	source, err := projectHelperSource("example.com/app/config", "/tmp/app")
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(source, "Config:      &configpkg.Config,") {
		t.Fatalf("helper source must pass native config pointer:\n%s", source)
	}
	if strings.Contains(source, "encoding/json") || strings.Contains(source, "json.") {
		t.Fatalf("helper source must not serialize gowdk.Config:\n%s", source)
	}
}
