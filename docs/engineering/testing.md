...
Use `node --check` when editor extension files change or before release-style verification.
...
- `internal/compiler` tests cover SSR addon enforcement, duplicate page/component identities, dynamic static routes requiring `paths`, static actions without SSR, and `load {}` rejection on static pages.
- `internal/discover` tests cover recursive `.gwdk` include/exclude matching.
- `internal/parser` tests cover page/component annotations, `paths`, `build`, `load`, `view`, `props`, `act`, captured `paths`/`build`/`view` bodies, the first action input/redirect body subset, and render mode rejection.
- `internal/view` tests cover static markup rendering, escaping, component expansion, and missing component/prop errors.
- `internal/staticgen` tests cover static HTML emission, literal build data, literal dynamic paths, route and asset manifest output, component expansion, nested route output paths, and no partial output on unsupported pages.
