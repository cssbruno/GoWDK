...
| Inline Go authoring | Allow optional Go code blocks inside `.gwdk` only when they extract to normal importable, formatted, testable package Go. Separate `.go` files remain supported and generated adapters remain glue. Saved default and `go spa {}` blocks are type-checked with sibling Go files during validation, default and `go spa {}` blocks can provide build-time no-argument functions for `build { => LocalFunc() }`, page-level `go spa {}` blocks can opt into browser Go by exporting `GOWDKMount<PageID>` for generated WASM page mounts, generated app source materializes default, `go spa {}`, and `go ssr {}` blocks under `gowdk_go/`, `go ssr {}` can provide generated SSR load handlers, and configured addons implementing `gowdk.GoBlockConsumer` can validate `go addon.<name> {}` blocks and emit generated app Go files. Source-adjacent extraction, richer SPA client seed wiring, and addon adapter contracts remain planned. | Partial |
