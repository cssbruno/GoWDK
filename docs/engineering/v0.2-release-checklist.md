...
- [x] `scripts/validate-release-notes.sh` validates the release note template and
  v0.2 draft release notes for experimental/not-production-ready warnings,
  implemented/partial/planned/out-of-scope/known-gaps sections, generated-output
  warning, checksum instructions, and attestation instructions.
- [x] `.github/workflows/release.yml` validates the selected release body before
  creating the draft pre-release.
- [x] `.github/workflows/release-smoke.yml` verifies published Linux, macOS
  Intel, macOS ARM, and Windows CLI artifacts by checksum and `gowdk version`.
- [x] `scripts/check-release-policy.sh` enforces core release policy invariants
  in CI.
- [x] The release workflow checks Go and Node gates, builds CLI artifacts,
  ...
  `checksums.txt`, and creates draft pre-releases.
