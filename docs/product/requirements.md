...
| PRD-013 | Complete request-time page rendering with `load {}`, guards, layouts, and error handling. | Medium | Partial | `addons/ssr` registers the SSR feature and provides load context, guard execution, route registration, request-aware layout composition, safe local redirect errors, and default error-handler contracts. Generated embedded apps can serve concrete and dynamic `@render ssr` pages rendered from `view {}` and literal or imported `build {}` data, generated SSR/action/API routes run declared guards with fail-closed missing-guard behavior, `load { => { field } }` execution calls same-package Go load functions through `ssr.LoadContext`, optional generated `404.html`/`500.html` pages are used by runtime app error responses, and generated SSR/action/API lanes have no-store panic boundaries; broader `load {}` data shapes and custom error-boundary syntax remain planned. |
| PRD-014 | Add optional WASM islands after the core compiler and action flow are stable. | Low | Partial | Explicit `g:island="wasm"` component calls emit WASM and loader assets under `assets/gowdk/islands/`. Declared `@wasm` browser-side Go packages are compiled with `GOOS=js GOARCH=wasm`, checked for browser-unsafe imports, and validated for required GOWDK ABI exports. Browser-runtime integration tests remain planned. |
