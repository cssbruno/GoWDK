...
data refresh, non-HTTP revalidation, richer worker/cron scheduler policies, and
...
| `runtime/i18n` | Typed message catalog helpers. | Runtime | Provides dependency-free typed catalog and bundle lookup with default-locale fallback plus simple named placeholder formatting for app-owned translated strings. |
