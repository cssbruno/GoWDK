...
| `internal/viewvalidation` | Validate view-bound client expressions. | Compiler | Owns DOM event symbols and typed validation for reactive attributes, `class:*`, and `style:*` expressions against `internal/clientlang`, without importing the renderer or runtime packages. `internal/view` keeps compatibility wrappers for older call sites. |
| `internal/view` | Render the first spa `view {}` markup subset. | Compiler | Lowercase HTML elements, spa/boolean/expression attributes, escaped text/attribute interpolation, self-closing component calls, prop/state interpolation, `g:on:*`, `g:island`, `g:command`, `g:query`, and query-bounded `g:subscribe` rendering implemented. Public parse, analysis, canonicalization, and validation helpers remain as compatibility wrappers around `internal/viewparse`, `internal/viewanalysis`, and `internal/viewvalidation`. |
...
| `addons/realtime` | Browser presentation-event fanout. | Addon | Feature registration and config/CLI wiring implemented. Exposes dependency-free SSE helpers from `runtime/contracts/sse`; WebSocket fanout remains in the nested `runtime/contracts/websocketfanout` module so its third-party dependency stays out of the root module graph. The compiler requires this feature for `g:subscribe` metadata, and generated apps use the SSE helper for subscription-filtered presentation streams. |
