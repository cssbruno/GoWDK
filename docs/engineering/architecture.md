...
| `addons/realtime` | Browser presentation-event fanout. | Addon | Feature registration and config/CLI wiring implemented. Exposes dependency-free SSE helpers from `runtime/contracts/sse`; WebSocket fanout remains in the nested `runtime/contracts/websocketfanout` module so its third-party dependency stays out of the root module graph. |
