...
- `Submit` must use one of the supported signatures:
  ...
  `func(context.Context, form.Values) (response.Response, error)`, or
  `func(context.Context, form.Data) (response.Response, error)`.
...
- Generated typed action decoders are built from same-package Go type metadata,
  ...
  signed integers, unsigned integers, `form.File`, and `[]form.File`, reject
  repeated scalar fields, leave missing or blank numeric/boolean/file fields as
  zero values, and return structured errors without submitted values.
...
- Direct `input type="file"` controls are accepted only inside
  `enctype="multipart/form-data"` `g:post` forms and only when each file
  control declares literal `g:max-file-size`, `g:max-files`, and MIME `accept`
  policy. Generated adapters enforce the action body cap plus file count, size,
  and submitted content-type allow-lists before user handlers run. Upload
  storage, scanning, persistence, cleanup, auth, and logging policy remain
...
- Multipart action uploads expose `form.File` values to typed handlers and
  `form.Data` to low-level handlers. User handlers must stream/store files
  during the request; generated cleanup can remove parser temporary files after
  the handler returns.
...
- `input type="file"` requires `enctype="multipart/form-data"` plus
  `g:max-file-size`, `g:max-files`, and MIME `accept`. Generated code enforces
  those limits and leaves storage to the Go handler.
