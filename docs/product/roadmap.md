...
- compiler-validated query-bounded `g:subscribe` metadata for presentation
  ...
  missing/invalid/non-web-role diagnostics. Presentation event envelopes now
  preserve optional audience labels, and the dependency-free SSE fanout can
  scope delivery through server-owned request audience labels.
