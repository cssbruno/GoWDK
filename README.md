...
| Localization | Early | `Config.I18N` expands generated page routes per locale, passes `BuildParams.Locale` to build helpers, attaches SSR route locale to `runtime/app.Locale(ctx)`, and provides typed Go catalogs through `runtime/i18n`. | Message extraction, ICU/plural/date formatting, and per-endpoint locale policy remain app-owned or planned. | [Routing](docs/reference/routing.md) | [i18n](examples/i18n/README.md) |
...
| Contracts | Works, contract unstable | Runtime contracts support typed queries, commands, events, jobs, role filtering, local dispatch, file outbox, broker/fanout adapters, worker replay with dedup/backoff hooks, standalone worker/cron role binaries, contract graph/trace/list commands, generated `g:command`/`g:query` web adapters, and explicit domain-event to query invalidation metadata. | Richer scheduler policies, deployment adapters, durable retry operations, and editor-first contract visualization remain planned platform tooling or app-owned infrastructure. | [Contracts](docs/reference/contracts.md) | [Runtime contracts](runtime/contracts) |
| Realtime | Works, contract unstable | `FeatureRealtime` and `addons/realtime` provide opt-in presentation-event fanout packaging, dependency-free SSE helpers with server-owned audience scoping, configurable retry hints, bounded replay, audience revocation, process-local stats, nested WebSocket transport docs, compiler-validated `g:subscribe` metadata, generated subscription-filtered guarded SSE fanout, a bounded generated client patch loop for query-owned regions, generated `gowdk.query.invalidate` events, and route/query refresh endpoint fallback for non-subscribed invalidated query regions. | Richer patch shapes, fragment/API-specific query execution, durable replay, and production telemetry export remain hardening work. | [Realtime](docs/reference/realtime.md) | [SSE](runtime/contracts/sse) |
