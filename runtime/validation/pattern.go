...
func (parser *patternParser) parseClassItem() ([][2]rune, error) {
	...
	if parser.peek() == '[' && parser.peekNext() == ':' {
		return nil, fmt.Errorf("POSIX character classes are not supported")
	}
	if parser.peek() == '\\' {
		parser.cursor++
		if parser.peek() == 0 {
...
func (parser *patternParser) parseEscape() (patternAtom, error) {
	...
	case 'p', 'P':
		return patternAtom{}, fmt.Errorf("Unicode character class escapes are not supported")
	default:
