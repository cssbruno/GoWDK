...
| WASM islands | Early | Component-level `wasm` and page-level `go client {}` emit Go `js/wasm` browser assets for supported fixtures; build-time validation checks browser-safe imports and ABI exports, and browser tests cover mount, event, patch, emit, destroy, and invalid patch rejection. | ABI docs/examples and user-code runtime validation beyond the current patch contract remain planned. | [Components](docs/language/components.md) | [Test fixture](testfixture/islands/islands.go) |
