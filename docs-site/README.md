...
Deployment is a static publish (see `render.yaml`): with `rootDir: docs-site`,
the host checks out the monorepo, runs syncdocs and the in-tree GOWDK build from
this directory, copies `assets/`, and serves `dist/site`. A static preview of any
