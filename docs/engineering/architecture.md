...
| `runtime/contracts` | Typed contract registry and in-process dispatch. | Runtime | First runtime slice implemented for queries, commands, backend-owned domain and integration events, presentation events, jobs, metadata, local command-buffered event dispatch, event-envelope capture/replay, and dependency-free outbox/broker/presentation-fanout interfaces. Compiler/generator integration and concrete reliability/realtime adapters remain planned. |
