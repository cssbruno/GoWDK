...
| PRD-012 | Support server fragments for partial updates without full-page SSR. | Medium | Partial | `addons/partial`, `runtime/response.FragmentFor`, generated client runtime emission, and first-slice generated action fragment responses for partial POSTs are implemented. CSRF, user action logic, validation fragments, richer fragment rendering, and local client-side reactivity remain planned. |
...
| PRD-022 | Allow generated app output to compile to a WASM deploy artifact. | Medium | Partial | `gowdk build --wasm <file>` and `Build.Targets[].WASM` compile the generated app with `GOOS=js GOARCH=wasm`. Browser WASM islands remain future work. |
