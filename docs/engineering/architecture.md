...
| `internal/staticgen` | Emit route-derived static HTML files for build-time pages and first-slice SSR render artifacts. | Compiler | Initial simple page, literal build data, imported Go build data calls, literal dynamic path expansion, component expansion, partial runtime asset emission, default JS island asset emission, explicit WASM island asset emission, simple concrete SSR page rendering, route manifest emission, asset manifest emission, identical-output write skipping, and incremental changed-page static rendering implemented. |
