...
| `internal/gwdkir` | Stable internal compiler IR shared by generated-output passes. | Compiler | IR records for packages, source files, page routes, backend endpoints, templates, client behavior, parsed view nodes, typed `paths {}`/`build {}` records, and asset scope/hash metadata implemented. Generated artifact planning stays in the consuming generator packages. |
...
| `internal/clientrt` | Own embedded browser runtime sources for partial updates, static-first SPA navigation, stores, JS islands, and WASM loaders. | Runtime | Framework-owned runtime source is kept as `.js` files under `internal/clientrt/assets/` and embedded with `go:embed`. First partial form enhancement runtime emits lifecycle hooks, target/swap request headers, swaps, focus restoration, loading state metadata, island remounts, and page-level `go client {}` remounts after SPA navigation. |
...
`build {}` source text, typed literal/build-call records, parsed view nodes, and
declared blocks. Current public manifest JSON is intentionally smaller: it
