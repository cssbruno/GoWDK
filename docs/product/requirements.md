...
| PRD-016 | Make hybrid pages an explicit request-time lane. | High | Implemented | `@render hybrid` pages require the SSR addon, are generated as request-time `hybrid` routes, and can render without `load {}` or with declared `load {}` data. |
...
| Hybrid | Treat `@render hybrid` as the explicit request-time branch; defer streaming, data refresh, and non-HTTP revalidation. | Partial |
