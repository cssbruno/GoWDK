...
- Build-time SPA/static output is the default page lane. `server {}` and
  `go server {}` opt pages into request-time rendering. Actions, APIs, and
...
The last two columns separate two kinds of gap. `Current Limit` lists
capabilities GOWDK intends to provide but has not stabilized. `App-owned by
design` lists boundaries that intentionally stay in app Go (or app/deploy
infrastructure) — deliberate ownership lines, not missing framework features.

| Surface | Status | Works Today | Current Limit | App-owned by design | Docs | Example |
| --- | --- | --- | --- | --- | --- | --- |
...
| Actions | Works, contract unstable | Generated apps can serve typed POST action handlers, decode supported form inputs including bounded multipart uploads, validate request shape, return redirects/fragments, and opt into CSRF. | Typed action result accessors are planned. | Upload storage/scanning/persistence and domain validation stay in app Go handlers. | [Actions](docs/language/actions.md) | [Login](examples/login/README.md) |
...
| SSR | Works, contract unstable | Pages with `server {}` or `go server {}` can build request-time handlers when the SSR addon is enabled. | Typed route-param accessors, lifecycle docs, and error/cache contracts need more hardening. | — | [SSR](docs/language/ssr.md) | [SSR](examples/ssr/simple-ssr.page.gwdk) |
...
| Generated app WASM | Early | `gowdk build --app --wasm` compiles the generated app into a Go `js/wasm` deploy artifact, and CI verifies the emitted module header. | Generated-app WASM packaging is still expanding, and is separate from component-level WASM islands. | Host runtime/loader integration is deploy-platform owned. | [Deployment](docs/reference/deployment.md) | [Embed](examples/embed/site.page.gwdk) |
...
| Observability | Early | `runtime/trace` provides dependency-free W3C trace IDs, context spans, sampling, sinks, source refs, `traceparent` propagation, a local JSON/SSE collector, and a self-contained viewer. `addons/observability` enables debug-only generated backend, SSR, load, frontend, and island tracing, and `runtime/trace/otel` provides an optional nested OTLP bridge. | Production-safe sampling and OTLP export primitives are still expanding. | Durable telemetry storage, hosted analysis, and production sampling/retention/access policy stay app-owned. | [Observability](docs/reference/observability.md) | [Runtime trace](runtime/trace) |
| Security audit | Early | `gowdk audit` derives an IR-backed posture for routes, endpoints, contracts, and frontend surface risks; evaluates the built-in baseline plus declared `*.audit.gwdk` policies; exits non-zero on error findings; can emit/run generated audit tests; and `gowdk build` runs the same baseline, blocking production builds on error findings unless `--allow-insecure` is set. | The audit DSL and generated tests cover the current baseline slice; richer role fixtures and deeper browser/data-flow analysis are planned. | Authentication, sessions, RBAC storage, and backend resource authorization stay in app Go. | [Security](docs/engineering/security.md) | [Spec](docs/product/security-audit-spec.md) |
...
| `observability` | Generated app tracing and a local trace viewer |
...
| `spa` | Static-first SPA navigation compatibility |
